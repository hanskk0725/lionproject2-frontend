...
export default function PaymentPage() {
  ...
  const handlePayment = async () => {
    ...
    try {
      ...
      // PortOne v2 응답에서 transactionId를 impUid로 사용
      const impUid = (response as any).transactionId || (response as any).imp_uid || response.paymentId;
      if (!impUid) {
        console.error('PortOne 응답:', response);
        alert('결제 ID를 찾을 수 없습니다. 관리자에게 문의해주세요.');
        setIsProcessing(false);
        return;
      }
